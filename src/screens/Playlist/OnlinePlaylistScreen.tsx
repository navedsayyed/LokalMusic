...
import { PlaylistDetail, getPlaylistById } from "@/services/api/music.api";
...
type OnlinePlaylistRouteParams = {
    playlistId: string;
...
/* ─── Main Screen ─────────────────────────────────────────────── */
export const OnlinePlaylistScreen = () => {
    const route = useRoute();
    const navigation = useNavigation<any>();
    const params = route.params as OnlinePlaylistRouteParams;
    ...
    useEffect(() => {
        ...
        setLoading(true);
        getPlaylistById(params.playlistId).then((data) => {
            setPlaylist(data);
            setLoading(false);
        });
    }, [params?.playlistId]);
    ...
    const coverUrl = playlist?.imageUrl ?? (params?.playlistImageUrl as string | undefined);
    const playlistName = playlist?.name ?? params?.playlistTitle ?? "Playlist";
        ...
        <>
                ...
                >
                    {coverUrl ? (
                        ...
                        <View style={[styles.heroImage, { backgroundColor: "#1a2340", alignItems: "center", justifyContent: "center" }]}>
                            <Ionicons name="musical-notes" size={80} color="#ffffff22" />
                        </View>
                    )}
                </Animated.View>
                ...
                <View style={styles.heroContent}>
                    ...
                    {playlist?.fanCount ? (
                        <Text style={styles.fanCount}>
                            {playlist.fanCount.toLocaleString()} followers
                        </Text>
                    ) : null}
                </View>
            </View>
            ...
            {!loading && songs.length === 0 && (
                <View style={{ alignItems: "center", marginTop: 40, gap: 10 }}>
                    ...
                    <Text style={{ color: palette.textSecondary, fontSize: 14 }}>No songs found</Text>
                </View>
            )}
        </>
    );
