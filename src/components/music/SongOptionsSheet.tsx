...
export const SongOptionsSheet: React.FC<Props> = ({
    ...
    const handleShare = async () => {
        try {
            await Share.share({
                message: `🎵 Listen to "${song.name}" by ${song.primaryArtists} on Mume!`,
                title: song.name,
            });
        } catch (_) { }
        onClose();
            ...
            >
                ...
                <View style={[styles.songPreview, { borderBottomColor: palette.border }]}>
                    {song.imageUrl ? (
                        ...
                        <View style={[styles.previewThumb, { backgroundColor: palette.backgroundSecondary }]}>
                ...
                >
                    {OPTIONS.map((opt) => (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.6}
                        >
                            <View style={[styles.optionIconWrap, { backgroundColor: palette.backgroundSecondary }]}>
                                <Ionicons
                                    name={opt.icon}
                                    size={20}
